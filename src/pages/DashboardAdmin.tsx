...
export default function DashboardAdmin() {
  ...
  useEffect(() => {
    ...
    const load = () => {
      setLoading(true);
      const usersReq = api.admin.users()
        .then((res) => {
          if (!mounted) return;
      ...
      const statsReq = api.admin.statistics()
        .then((res) => {
          if (!mounted) return;
          setStats({
            totalUsers: res.data.statistics.totalUsers,
            totalLessons: res.data.statistics.totalLessons,
            totalQuizzes: res.data.statistics.totalQuizzes
          });
        })
        .catch((err) => {
          if (!mounted) return;
      ...
      Promise.allSettled([usersReq, lessonsReq, quizzesReq, statsReq]).finally(() => {
        if (!mounted) return;
        setLoading(false);
      });
    };
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="text-gray-700">
                      {userRows.map((row, index) => (
                        <tr key={row.id || `${row.name}-${row.email}-${index}`}>
