          ...
          <div className="animate-fadeInUp">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
              <div>
                ...
                <p className="text-gray-600 mt-3">Explore interactive lessons at your own pace.</p>
              </div>
              <div className="flex gap-3">
                <Link to="/quiz" className="bg-primary text-white px-6 py-3 rounded-md font-semibold hover:bg-blue-700 transition-all duration-300">
                  Go to Quizzes
                </Link>
              </div>
            </div>
              ...
              <div className="grid md:grid-cols-3 gap-4 text-sm">
                {categories.map((item) => (
                  <button
                    ...
                    className={item === selectedCategory
                        ? 'border-2 border-primary text-primary rounded-lg px-4 py-3 font-semibold text-left'
                        : 'border border-gray-200 rounded-lg px-4 py-3 text-gray-600 text-left hover:border-primary transition'
                    }
                  >
                    {item}
                  </button>
            ...
            <div className="grid lg:grid-cols-[300px_1fr] gap-6">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                <h3 className="font-bold mb-4">Lesson Menu</h3>
                <div className="grid gap-3 text-sm">
                  {filteredLessons.map((item) => (
                    <button
                      key={item._id}
                      onClick={() => setSelectedLessonId(item._id)}
                      className={item._id === lesson?._id
                          ? 'border-2 border-primary text-primary rounded-lg px-4 py-3 font-semibold text-left'
                          : 'border border-gray-200 rounded-lg px-4 py-3 text-gray-600 text-left hover:border-primary transition'
                      }
                    >
