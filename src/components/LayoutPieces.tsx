        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-4 text-sm">
            {(items || []).map((item, index) => (
              <Link key={`${item.label}-${index}`} to={item.to} className={item.className || 'text-gray-600 hover:text-primary'}>
                {item.label}
              </Link>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-3">
              {(items || []).map((item, index) => (
                <Link key={`${item.label}-${index}`} to={item.to} className={item.className || 'text-gray-700 hover:text-primary transition-colors font-medium'}>
                  {item.label}
                </Link>
              ))}
            </div>
